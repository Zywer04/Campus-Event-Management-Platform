...
const App: React.FC = () => {
    ...
    const [selectedFields, setSelectedFields] = useState<string[]>(['name', 'startTime', 'endTime', 'location', 'status', 'participants']);
    ...
    const handleSaveEdit = () => {
        ...
        // Update current activity if detail sidebar is open
  ...
  const clubMenus = [
    ...
    const handleDelete = () => {
        ...
        setSelectedActivities([]);
        setShowDetailSidebar(false); // 如果是详情页删除，也关闭
    };
    ...
    const filteredActivities = activities.filter(activity => {
        ...
        const matchesOrganizer = selectedOrganizer === 'all' || activity.organizer === selectedOrganizer;
        if (!matchesOrganizer) return false;
                            ...
                            {categories.map(category => (
                                <button
                                    ...
                                    className={`w-full flex items-center space-x-3 px-4 py-2 rounded-lg text-left transition-colors cursor-pointer whitespace-nowrap !rounded-button ${
                                        location.pathname === '/' && location.search === `?category=${category.id}`
                        ...
                        <div className="space-y-2">
                            {myActivities.map(item => (
                                <button
                                    ...
                                    className={`w-full flex items-center space-x-3 px-4 py-2 rounded-lg text-left transition-colors cursor-pointer whitespace-nowrap !rounded-button ${
                                        location.pathname === (item.id === 'history' ? '/activities/history' : '/registered')
                                            ? 'bg-purple-50 text-purple-600'
                                            : 'text-gray-600 hover:bg-gray-50'
                                    }`}
                                >
                        ...
                        <div className="space-y-2">
                            {adminMenus.map(item => (
                                <button
                                    ...
                                    onClick={() => handleNavigation(item.id)}
                                    className={`w-full flex items-center space-x-3 px-4 py-2 rounded-lg text-left transition-colors cursor-pointer whitespace-nowrap !rounded-button ${
                                        location.pathname === (item.id === 'manage' ? '/ActivityManage' : 
                                            item.id === 'publish' ? '/publish' : 
                                            item.id === 'stats' ? '/Stats' : '/AuditPage')
                                            ? 'bg-purple-50 text-purple-600'
                                            : 'text-gray-600 hover:bg-gray-50'
                                    }`}
                                >
                        ...
                        <div className="space-y-2">
                            {clubMenus.map(item => (
                                <button
                                    ...
                                    onClick={() => handleNavigation(item.id)}
                                    className={`w-full flex items-center space-x-3 px-4 py-2 rounded-lg text-left transition-colors cursor-pointer whitespace-nowrap !rounded-button ${
                                        location.pathname === (item.id === 'clubActivities' ? '/ClubActivities' : 
                ...
                <div className="max-w-[1176px] mx-auto px-6 py-8">
                    ...
                    <div className="bg-white rounded-xl shadow-sm p-6 mb-6">
                        <div className="flex flex-wrap items-center gap-4">
                            ...
                            <div className="relative">
                                <button
                                    onClick={() => setShowOrganizerFilter(!showOrganizerFilter)}
                                    className={`flex items-center space-x-2 px-4 py-2 bg-white rounded-lg border ${selectedOrganizer !== 'all' ? 'border-purple-500 text-purple-600' : 'border-gray-200 text-gray-700'
                                        } hover:border-purple-500 cursor-pointer whitespace-nowrap !rounded-button`}
                                >
                                    <i className={`fas fa-building ${selectedOrganizer !== 'all' ? 'text-purple-600' : 'text-gray-400'}`}></i>
                                    <span>举办部门</span>
                                    <i className={`fas fa-chevron-down text-xs ${selectedOrganizer !== 'all' ? 'text-purple-600' : 'text-gray-400'}`}></i>
                                </button>
                                {showOrganizerFilter && (
                                    <div className="absolute top-full left-0 mt-2 w-60 bg-white rounded-lg shadow-lg py-2 z-50">
                                        {['全部部门', '学生会文艺部', '创新创业学院', '体育部', '计算机学院学生会', '校团委', '毕业生委员会'].map((dept, i) => (
                                            <div
                                                key={i}
                                                className="px-4 py-2 hover:bg-purple-50 cursor-pointer"
                                                onClick={() => {
                                                    setSelectedOrganizer(dept === '全部部门' ? 'all' : dept);
                                ...
                                <div className="mb-6">
                                    ...
                                    <div className="space-y-2">
                                        ...
                                        <label className="flex items-center space-x-2">
                                            <input
                                                type="checkbox"
                                                checked={selectedFields.includes('participants')}
                                                onChange={(e) => {
                                                    if (e.target.checked) {
                                                        setSelectedFields([...selectedFields, 'participants']);
                                                    } else {
                                                        setSelectedFields(selectedFields.filter(f => f !== 'participants'));
                                                    }
                                                }}
                                                className="h-4 w-4 text-purple-600 rounded border-gray-300 focus:ring-purple-500"
                                            />
